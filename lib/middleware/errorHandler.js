...
module.exports = function(options) {
  ...
  return function errorHandler(err, req, res, next) {
    if (mode == 'direct') {
      ...
      // Hardcode    
      res.statusCode = err.status < 500 ? 200: err.status;
      if (err.status) { e.code = err.status; }
      ...
      /*if (err.uri) { 
        newErr.error_uri = err.uri; 
      }*/
      e.errors.push(newErr);
      
      // Construct message
      e.message = `${newErr.field}`;
